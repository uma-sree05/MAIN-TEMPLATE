...
(function ($) {
    ...
    const scrollTopPercentage = ()=> {
        const scrollPercentage = () => {
            ...
            scrollElementWrap.css("background", `conic-gradient( #ff5722 ${scrollValue}%, #080A0B ${scrollValue}%)`);
            ...
            if( scrollValue < 96 ) {
                ...
                $("#scroll-percentage-value").html('<i class="fa fa-angle-up"></i>');
            }
        }
...
$(document).ready(function (e) {
    console.log("here");
    var token = "0960d5d3-189e-40c7-9b70-725f1224f5df";
    var ipAddress = "";
    var formID = 2781;
    $.ajax({
        type: "GET",
        ...
        url: 'https://www.pringleapi.com/api/MyIpAddress',
    ...
    $("#btnSubmit").click(function () {
        ...
        { 
            var errorMessage = "Confirm you are not a robot";
        ...
        else
        {
            ...
            if (document.getElementById("txtCompanyName").value === "") {
                errorMessage += "<li>Company name on Credit Card is Required</li>";
                document.getElementById("businessName").focus();
            }
            
            if (document.getElementById("txtNote").value === "") {
                errorMessage += "<li>Note is Required</li>";
                document.getElementById("txtEmailAddress").focus();
            }
            ...
            var customer = {
                ...
                Fax: "1111111111",
                OnGoingSpecials: 1,
                CustomerCategory: 'Free Demo Customer - Business'
            };
            ...
            $.ajax({
                ...
                url: 'https://pringleapi.com/api/Crm',
                beforeSend: function (hdr) {
                    hdr.setRequestHeader('Token', token);
                },
                success: function (customerId) {
                    if (customerId != "") {
                        ...
                        $.ajax({
                            ...
                            beforeSend: function (hdr) { hdr.setRequestHeader('Token', token); },
                            success: function (reservationId) {
                                if (reservationId != "") {
                                    // Form Result Storage    
                                    var formResult = { ReservationID: reservationId, FormID: formID };   
                                    $.ajax({
                                        type: "POST",
                                        dataType: "json",
                                        contentType: "application/json",
                                        data: JSON.stringify(formResult),
                                        url: "https://www.pringleapi.com/api/formresult",
                                        beforeSend: function (hdr) { hdr.setRequestHeader('Token', token); },
                                        success: function (formResultID) {
                                            var reservationAttributeLst = new Array();
                                            var currentSectionName = "";
                                            var formResult = "<table width='100%' cellpadding=4 cellspacing=2 align='center'>";
                                            formResult += "<tr><td bgcolor='lemonchiffon' colspan=2><b>Personal Information</b></td></tr>";
                                            formResult += "<tr><td>First Name:</td><td>" + $("#txtFirstName").val() + "</td></tr>";
                                            formResult += "<tr><td>Last Name:</td><td>" + $("#txtLastName").val() + "</td></tr>";
                                            formResult += "<tr><td>Email:</td><td>" + $("#txtEmailAddress").val() + "</td></tr>";
                                            formResult += "<tr><td>Phone:</td><td>" + $("#txtWorkPhone").val() + "</td></tr>";
                                            formResult += "<tr><td>Company Name:</td><td>" + $("#txtCompanyName").val() + "</td></tr>";
                                            formResult += "<tr><td>Note:</td><td>" + $("#txtNote").val() + "</td></tr>";
                                            formResult += "</table>";
                                            var arrayIndex = 0;
                                            reservationAttributeLst[arrayIndex] = {
                                                ReservationId: reservationId,
                                                AttributeName: "Form Results",
                                                AttributeValue: formResult
                                            };
                                            $.ajax({
                                                type: "POST",
                                                dataType: "json",
                                                ...
                                                success: function (reservationAttributeID) {
                                                    $.ajax({
                                                        ...
                                                        dataType: "json",
                                                        contentType: "application/json",
                                                        url: 'https://pringleapi.com/api/FormResult?reservationID=' + reservationId + "&formID=" + formID,
                                                        beforeSend: function (hdr) {
                                                            hdr.setRequestHeader('Token', token);
                                                        },
                                                        success: function (sendformresultsobject) {
                                                            $("#successMessage").html("<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Thank you for your interest in Pristine. One of our associates will get back to you shortly.");
                                                            $("#successMessage").addClass("alert alert-success alert-dismissable");
                                                            $("#frmScheduleADemo")[0].reset()
                                                        },
                                                        error: function (error, status) {
                                                        }
                                                    });
